...
export const WatchlistRow: React.FC<WatchlistRowProps> = ({
  ...
  const trendData = useMemo(() => {
    ...
    const result = [];
    const openPrice =
      liveData.priceInfo.open || liveData.priceInfo.previousClose;
    ...
    for (let i = 0; i <= pts; i++) {
      ...
      current +=
        (endPrice - startPrice) / pts + (pr - 0.5) * (startPrice * 0.002);
  ...
  const perfPct = Math.min(
    Math.max(((curPr - wkMin) / (wkMax - wkMin)) * 100, 0),
    100,
  );
  ...
  const dummyVol = useMemo(() => {
    ...
    const mult =
      ((ticker.symbol.charCodeAt(0) + ticker.symbol.charCodeAt(1)) % 10) + 1;
    ...
    >
      ...
      >
        ...
        <div className="overflow-hidden">
          ...
          <p className="text-[9px] uppercase font-bold text-gray-500 dark:text-gray-400 mt-0.5 tracking-widest">
      ...
      <div
        className="w-[15%] hidden lg:block text-right cursor-pointer"
        onClick={onClick}
      >
      ...
      <div
        className="w-[10%] hidden xl:flex items-center gap-2 justify-end cursor-pointer"
        onClick={onClick}
      >
