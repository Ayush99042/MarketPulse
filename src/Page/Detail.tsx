  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <span className="text-xs font-black text-gray-500 dark:text-gray-400/70 uppercase tracking-[0.2em]">
          {label}
        </span>
...
export const Detail: React.FC = () => {
  ...
  useEffect(() => {
    if (livePrice && liveData?.priceInfo && intradayData.length === 0) {
      ...
      if (durationMs <= 0) {
        setIntradayData([
          { timestamp: startTime, close: startPrice, isSimulated: true },
        ]);
        return;
      }
  ...
  const chartData = useMemo(() => {
    ...
    switch (duration) {
      case "1W":
        return chronData.slice(-5);
      case "1M":
        return chronData.slice(-22);
      case "3M":
        return chronData.slice(-66);
      case "6M":
        return chronData.slice(-132);
      case "1Y":
        return chronData.slice(-252);
      default:
        return chronData;
    }
  ...
  const chartTicks = useMemo(() => {
    ...
    return Array.from(
      { length: count },
      (_, i) => first + ((last - first) * i) / (count - 1),
    );
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <Card
          glass={true}
          className="p-4 border-white/20 dark:border-white/10 shadow-2xl backdrop-blur-3xl"
        >
      ...
      <div className="container mx-auto p-8 space-y-4">
        ...
        <Card glass={true} className="p-12 text-center text-rose-500">
          ...
          <p className="opacity-70 text-lg">
            Unable to retrieve institutional records for {symbol}.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 lg:px-8 pt-2 space-y-2 animate-in fade-in slide-in-from-bottom-6 duration-1000 ease-out">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-2 mb-5">
          <div className="space-y-0.5">
            ...
            <div className="flex flex-col">
              <h1 className="text-xl lg:text-3xl font-black text-gray-900 dark:text-white leading-[1.05]">
                {fullName}
              </h1>
            ...
            <div className="flex flex-col items-end">
              {livePrice !== null ? (
                <div className="text-right">
                  ...
                  {liveData && (
                    <div
                      className={`text-sm lg:text-md font-black mt-1 ${liveData.priceInfo.pChange >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                    >
                      {liveData.priceInfo.pChange >= 0 ? "↗" : "↘"}{" "}
        ...
        <Card
          glass={true}
          className="p-4 lg:p-5 border-white/20 dark:border-white/5 shadow-xl"
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-1">
            <div className="space-y-0">
              ...
              <p className="text-[8px] uppercase font-black tracking-[0.3em] text-gray-500/60 ml-8">
            ...
            <div className="flex flex-wrap gap-2 p-1.5 bg-black/5 dark:bg-white/5 rounded-[1.5rem] backdrop-blur-xl border border-white/10">
              {(["1D", "1W", "1M", "3M", "6M", "1Y"] as Duration[]).map((d) => (
                <button
                  ...
                  className={`px-6 py-2.5 text-xs font-black rounded-2xl transition-all duration-500 uppercase tracking-widest ${
                    duration === d
              ...
              >
                <defs>
                  <linearGradient id="colorPrice" x1="0" y1="0" x2="0" y2="1">
                    <stop
                      offset="5%"
                      stopColor={direction === "up" ? "#10b981" : "#f43f5e"}
                      stopOpacity={0.3}
                    />
                    <stop
                      offset="95%"
                ...
                <XAxis
                  ...
                  domain={
                    duration === "1D"
                      ? [marketBounds.start, marketBounds.end]
                      : ["dataMin", "dataMax"]
                  }
                  ticks={chartTicks}
                  tickFormatter={(val) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 pt-4">
          <div className="lg:col-span-2 space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <Card
                glass={true}
                className="p-10 border-white/20 dark:border-white/5"
              >
              ...
              <Card
                glass={true}
                className="p-10 border-rose-500/10 dark:border-white/5 group relative overflow-hidden"
              >
                ...
                <div className="flex justify-between items-center mb-10 relative z-10">
                  <h3 className="text-xs font-black text-rose-500/80 uppercase tracking-[0.4em]">
                    Yield Momentum
                  </h3>
                  <div className="flex gap-2 p-1 bg-black/5 dark:bg-white/5 rounded-2xl border border-white/5">
                    <button
                      onClick={() => setPerfRange("monthly")}
                      className={`text-[10px] px-3.5 py-1.5 rounded-xl font-black transition-all ${perfRange === "monthly" ? "bg-blue-600 text-white shadow-lg" : "text-gray-500"}`}
                    >
                      MO
                    </button>
                    <button
                      onClick={() => setPerfRange("yearly")}
                      className={`text-[10px] px-3.5 py-1.5 rounded-xl font-black transition-all ${perfRange === "yearly" ? "bg-blue-600 text-white shadow-lg" : "text-gray-500"}`}
                    >
                      YR
                    </button>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 relative z-10">
                  <div className="text-7xl font-black text-gray-900 dark:text-white mb-3">
                    {perfRange === "monthly" ? "+4.8%" : "+12.4%"}
                  </div>
                  <div className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.5em] mb-10">
                    Growth Alpha
                  </div>

                  <div className="w-full grid grid-cols-2 gap-6 border-t border-white/10 pt-10">
                    <div className="text-center">
                      <div className="text-[9px] text-gray-500 uppercase tracking-[0.25em] mb-2 font-black">
                        Risk Profile
                      </div>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-base font-black text-blue-500 italic">
                        Optimistic
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                },
                {
                  ...
                  value: liveData?.metadata.pdSymbolPe || 24.5,
                },
                {
                  label: "Weighted VWAP",
                  value: liveData?.priceInfo.vwap?.toFixed(2) || "-",
                },
                {
                  label: "Liquidity Vol",
                ...
                <Card
                  key={i}
                  glass={true}
                  className="p-6 border-white/20 dark:border-white/5 hover:scale-[1.05] transition-all duration-500 group"
                >
                  <div className="text-[9px] font-black text-gray-500/80 uppercase mb-4 tracking-[0.3em] group-hover:text-blue-500 transition-colors">
                    {stat.label}
                  </div>
                  <div className="text-xl font-black text-gray-900 dark:text-gray-100 leading-none">
                    {stat.value}
                  </div>
                </Card>
              ))}
            ...
            <Card
              glass={true}
              className="p-10 lg:p-14 border-white/20 dark:border-white/5"
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-12 gap-x-16">
                {[
                  ...
                  },
                  {
                    ...
                    value: liveData?.metadata.pdSectorInd || "-",
                  },
                ].map((stat, i) => (
                  <div key={i} className="group relative">
                    <div className="text-[10px] text-gray-500/70 uppercase font-black mb-2 tracking-[0.2em] group-hover:text-blue-500 transition-colors">
                      {stat.label}
                    </div>
                    <div className="text-base font-black text-gray-900 dark:text-gray-100 font-mono ">
                      {stat.value}
                    </div>
                  </div>
                ))}
            ...
            <Card
              glass={true}
              className="overflow-hidden border-white/20 dark:border-white/5"
            >
              <div className="p-8 bg-black/5 dark:bg-white/5 flex justify-between items-center border-b border-white/10 dark:border-white/5">
                <h3 className="text-sm font-black text-gray-500 uppercase tracking-[0.4em]">
                  Historical Ledger
                </h3>
              ...
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader className="bg-transparent border-white/5">
                    <Th className="text-[10px] uppercase tracking-[0.3em] font-black text-gray-400 py-6">
                      Timestamp
                    </Th>
                    <Th className="text-[10px] uppercase tracking-[0.3em] font-black text-gray-400 py-6 text-right">
                      Opening
                    </Th>
                    <Th className="text-[10px] uppercase tracking-[0.3em] font-black text-emerald-500/70 py-6 text-right">
                      Maximus
                    </Th>
                    <Th className="text-[10px] uppercase tracking-[0.3em] font-black text-rose-500/70 py-6 text-right">
                      Minimus
                    </Th>
                    <Th className="text-[10px] uppercase tracking-[0.3em] font-black text-blue-500/70 py-6 text-right">
                      Finality
                    </Th>
                  </TableHeader>
                  <TableBody>
                    {data?.slice(0, 30).map((record, i) => (
                      ...
                      >
                        <Td className="text-xs font-black text-gray-500 py-5">
          ...
          <div className="relative">
            <div className="space-y-10 sticky top-28 self-start transition-all duration-300">
              <Card
                glass={true}
                className="p-10 border-white/20 dark:border-white/5 relative overflow-hidden group"
              >
                ...
                <div className="space-y-10 relative z-10">
                  ...
                  <div className="space-y-6 pt-10 border-t border-white/15">
                    {[
                      ...
                      },
                      {
                        ...
                        value: liveData?.info.segment || "EQUITY",
                      },
                    ].map((item, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center bg-black/5 dark:bg-white/5 p-4 rounded-[1.25rem] border border-white/5"
                      >
                        <span className="text-[9px] uppercase font-black text-gray-500 tracking-[0.25em]">
                          {item.label}
                        </span>
                  ...
                  <div className="p-6 bg-gradient-to-br from-blue-600/15 to-transparent rounded-[1.5rem] border border-white/10 text-center shadow-inner">
                    <div className="text-[9px] font-black text-blue-500 uppercase mb-3 tracking-[0.3em]">
                      ISIN Protocol Hub
                    </div>
              ...
              <Card
                glass={true}
                className="p-10 bg-blue-600/20 text-white border-blue-500/20 relative overflow-hidden group shadow-2xl"
              >
                ...
                <div className="relative z-10 space-y-8">
                  <div>
                    <h3 className="text-[11px] font-black uppercase tracking-[0.5em] mb-2 text-blue-200">
                      Terminal Pulse
                    </h3>
                    <div className="text-[10px] font-bold text-blue-200/60 uppercase italic tracking-widest">
                      Global Status Node
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center bg-white/10 p-5 rounded-[2rem] border border-white/10 backdrop-blur-3xl shadow-xl">
                      ...
                      <span className="text-xs font-black uppercase tracking-widest bg-white text-blue-600 px-5 py-1.5 rounded-full shadow-lg">
                        {liveData?.metadata.status || "Active"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center px-4">
                      ...
                      <span className="text-3xl font-black italic text-white underline decoration-white/30 underline-offset-[12px]">
                        {liveData?.metadata.series || "EQ"}
                      </span>
                    </div>
                  </div>
